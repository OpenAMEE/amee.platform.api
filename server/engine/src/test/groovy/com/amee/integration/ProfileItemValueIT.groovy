...
class ProfileItemValueIT extends BaseApiTest {
    ...
    def units(version) {
        if (version >= 3.6) {
            ...
            assertEquals 1, responseGet.data.item.output.amounts.amount.size()
            def amount = responseGet.data.item.output.amounts.amount[0]
            ...
            assertEquals SUCCESS_OK.code, responseGet.status
            assertEquals 1, responseGet.data.item.output.amounts.amount.size()
            amount = responseGet.data.item.output.amounts.amount[0]
            assertNotNull amount
